...
class Corpus
  ...
  # Build a bigrams_with_count for collocations. Iterate through every sentence, take
      ...
      0.upto(words.length - 2) {
        ...
        if ((type_of_word == @noun) || (type_of_word == @verb) || (type_of_word == @adjective))
          (j+1).upto(words.length - 1){
            |k|
            if (@noun == (get_type_of_word words[k])) #there should be a method to put a pair of words to callocation bigrams_with_count
